...
CAM_URL = "http://10.114.33.110:81/stream"  # port 81 = ESP32-CAM MJPEG stream port
                                             # (now only used by camera_server.py -
                                             #  the GUI itself no longer talks HTTP)

# The GUI no longer connects to the ESP32-CAM directly. camera_server.py
# (a separate process) does that, decodes the MJPEG stream, and republishes
# raw frames over this local shared-memory socket via GStreamer's
# shmsink/shmsrc elements - zero-copy on the same machine, same idea as
# xvimagesink writing straight into the X server's backing store (see
# module docstring). Both this file and camera_server.py must agree on
# this path.
VIDEO_SHM_SOCKET_PATH = "/tmp/litewing_video.sock"
...
class DroneVideoApp(Gtk.Window):
    ...
    def _build_ui(self):
        ...
        self.video_status_lbl = Gtk.Label(label=f"CAM (via server): {VIDEO_SHM_SOCKET_PATH}  //  connecting...")
    ...
    def _init_gamepad(self):
        ...
        self._gamepad_watch_id = GLib.io_add_watch(
            self._gamepad_channel, GLib.IOCondition.IN, self._on_gamepad_event)
    ...
    def _build_video_pipeline(self):
        ...
        # Source is now the local camera_server.py process via shared
        # memory, not the ESP32 directly - decode already happened there.
        # shmsrc receives caps automatically over the shm control channel,
        # so no explicit width/height/format needs to be declared here.
        if self.video_hud_enabled:
            pipe_str = (
                f'shmsrc socket-path="{VIDEO_SHM_SOCKET_PATH}" is-live=true do-timestamp=true '
                f'! videoconvert '
    ...
    def _start_video_delayed(self):
        ...
        # Reset the frame counter on every rebuild (initial start, HUD
        # toggle, or reconnect). Without this, the counter accumulates
        # across rebuilds, so the first watchdog check after any rebuild
        # would always compare a stale nonzero count against the -1
        # sentinel and falsely report frames flowing even if nothing new
        # had arrived from the new pipeline yet - the same bug found and
        # fixed in camera_server.py's watchdog.
        self._frame_count = 0
        try:
            self.video_pipeline = self._build_video_pipeline()
            self.video_pipeline.set_state(Gst.State.PLAYING)
            self.video_running = True
            self.video_status_lbl.set_text(f"CAM (via server): {VIDEO_SHM_SOCKET_PATH}  //  LIVE")
    ...
    def toggle_camera(self):
        ...
        if self._video_user_stopped:
            ...
            self.video_status_lbl.set_text(
                f"CAM (via server): {VIDEO_SHM_SOCKET_PATH}  //  connecting...")
            self.video_status_lbl.set_name("status_off")
            self._start_video_delayed()
        else:
            # -> STOP
...
def main():
    global DRONE_URI, VIDEO_SHM_SOCKET_PATH
    parser = argparse.ArgumentParser(
        description="LiteWing drone control GUI - video comes from camera_server.py "
                    "via shared memory, not directly from the ESP32-CAM.")
    parser.add_argument("--drone", default=DRONE_URI,
                        help=f"Drone link URI (default: {DRONE_URI})")
    parser.add_argument("--video-socket", default=VIDEO_SHM_SOCKET_PATH,
                        help=f"Shared-memory socket path camera_server.py is publishing to "
                             f"(default: {VIDEO_SHM_SOCKET_PATH})")
    args = parser.parse_args()
    DRONE_URI = args.drone
    VIDEO_SHM_SOCKET_PATH = args.video_socket

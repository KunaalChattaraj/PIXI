...
class TiltReferencePainter:
    ...
    This runs inside the pipeline itself, on GStreamer's own thread, so the
    marks get baked into the actual video pixels before they reach
    xvimagesink. That's what keeps this compatible with xvimagesink's
    hardware overlay - GTK-widget-layer transparency (a separate window
    composited on top) conflicts with Xv's hardware overlay and breaks
    video rendering, since Xv expects the window to use the display's
    normal opaque visual, not an alpha-enabled one.

    Call set_attitude(roll, pitch) from anywhere (GTK thread is fine) to
    update the live values - paint() is called by GStreamer for every
    frame and just reads whatever the latest values are."""
    ...
    def _stroke(self, cr, x1, y1, x2, y2, width=2.0, color=(0.0, 1.0, 0.53)):
        """Draws a line with a dark outline underneath so it stays visible
        over any video content, bright or dark."""
    ...
    def _show_outlined_text(self, cr, x, y, text, rotation, font_size):
        """Draws text with a dark outline for readability over any video
        content, centered at (x, y) and rotated to follow the arc."""
        cr.save()
        cr.translate(x, y)
        cr.rotate(rotation)
    ...
    def paint(self, cr, frame_width, frame_height):
        ...
        radius = min(frame_width, frame_height) * 0.16  # smaller overall
        scale = radius / 90.0  # scales every fixed-pixel part below together
        ...
        # Roll arc ticks + labels (rotate with roll only, not pitch) - uses
        # its own larger radius, separate from the small horizon ball, so
        # labels have real arc-length between them instead of crowding
        # together on the same tight little circle. Capped against cy so
        # the topmost label ("0") can never run past the top edge of the
        # frame no matter the video resolution.
        label_reach = 60  # rough fixed space needed for tick + offset + text beyond arc_radius
        ...
        for mag in (0, 10, 20, 30, 40):
            for sign in ((1,) if mag == 0 else (-1, 1)):
                ...
                is_labeled = mag % 20 == 0  # label every 20deg, tick every 10deg -
                                              # labeling every 10deg overlaps at the
                                              # low resolutions typical of ESP32-CAM streams
        ...
        # Fixed pointer (never rotates - always reads current bank) - sits
        # just above the roll arc, not the small horizon ball.
        ...
        cr.fill()
        _pointer_path(0)
        cr.set_source_rgb(1.0, 0.82, 0.05)  # yellow - distinct from green (moving) and red (chevron)
        cr.fill()

        # Fixed center chevron marker (never moves) - same yellow as the
        # pointer, since both are fixed references (unlike the green
        # horizon/arc/ladder, which move with roll/pitch).
        cw = 2.2 * scale
...
class DroneVideoApp(Gtk.Window):
    ...
    def _build_video_pipeline(self):
        ...
        # cairooverlay draws the tilt reference directly onto each frame's
        # pixels, inside the pipeline - this is what avoids needing any
        # GTK-widget-layer transparency (which conflicts with xvimagesink's
        # hardware overlay and was what broke video rendering before).
    ...
    def _on_overlay_caps_changed(self, overlay, caps):
        # Fires when the pipeline negotiates the actual frame size - keep it
        # so _on_overlay_draw knows where the frame's center is.
    ...
    def _on_overlay_draw(self, overlay, cr, timestamp, duration):
        # Runs on GStreamer's own thread, once per frame - reads whatever
        # roll/pitch values were last set (no GTK calls happen here, so no
        # thread-marshaling is needed, unlike anything that touches widgets).

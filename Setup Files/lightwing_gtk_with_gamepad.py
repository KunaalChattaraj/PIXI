...
# Flip any channel that reads backwards on your hardware. Throttle (ABS_RZ)
# was confirmed reversed during bench testing.
GAMEPAD_AXIS_INVERT = {
    "ABS_X": False,   # CH1 - roll
    "ABS_Y": False,   # CH2 - pitch
    "ABS_Z": False,   # CH3 - yaw
    "ABS_RZ": True,   # CH4 - throttle (reversed on this unit)
}
...
class DroneVideoApp(Gtk.Window):
    ...
    # ======================================================================
    # Same non-blocking pattern as everything else in this app: the evdev
    # file descriptor is watched via GLib.io_add_watch instead of a
    # blocking read_loop() on its own thread. That means gamepad input
    # arrives as just another GTK-main-loop event - no 5th thread added to
    # the 4 already juggled here (see module docstring), so no new GIL
    # contention risk of the kind that caused the video-freeze bug this
    # app already works around.
    #
    ...
    def _init_gamepad(self):
        ...
        self._gamepad_watch_id = GLib.io_add_watch(
            self.gamepad_dev.fd, GLib.IO_IN, self._on_gamepad_event)
        self.gui_log(f"[GAMEPAD] Watching {dev.path} ({dev.name!r}). "
    ...
    def _on_gamepad_event(self, fd, condition):
        """Runs on the GTK main thread (GLib called us because the fd has
        data ready) - safe to touch self.setpoint / widgets directly here,
        same as any other GTK callback. Must return True to keep being
        watched."""
        try:
    ...
    def _apply_gamepad_axis(self, axis_code, raw_value):
        ...
        if axis_code == ev_ecodes.ABS_X:
            ...
            norm = self._normalize_gamepad_axis(raw_value, "ABS_RZ")
            # norm is -1..+1 (stick down..up) -> map onto THRUST_MIN..THRUST_MAX
            self.setpoint["thrust"] = int(
                THRUST_MIN + (norm + 1.0) / 2.0 * (THRUST_MAX - THRUST_MIN))
        else:

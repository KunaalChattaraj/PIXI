...
#hud_toggle { background-color: rgba(10, 10, 10, 0.75); color: #00ff88; border: 1px solid #00aa55;
              font-size: 10px; font-weight: bold; padding: 6px 10px; }
#hud_panel { background-color: rgba(5, 15, 10, 0.72); border: 1px solid #00aa55; padding: 10px 16px; }
#hud_readout { color: #00ff88; font-size: 14px; font-weight: bold; }
#hud_readout_dim { color: #558866; font-size: 12px; }
"""
...
class DroneVideoApp(Gtk.Window):
    ...
    def _build_ui(self):
        ...
        # ---------------- Overlay: HUD (heads-up telemetry over video) ----
        self.hud_revealer = Gtk.Revealer()
        self.hud_revealer.set_transition_type(Gtk.RevealerTransitionType.CROSSFADE)
        self.hud_revealer.set_transition_duration(200)
        self.hud_revealer.set_halign(Gtk.Align.START)
        self.hud_revealer.set_valign(Gtk.Align.END)
        self.hud_revealer.set_margin_start(16)
        self.hud_revealer.set_margin_bottom(16)
        self.hud_revealer.set_reveal_child(False)  # start hidden

        hud_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=4)
        hud_box.set_name("hud_panel")

        self.hud_status_lbl = Gtk.Label(label="DISCONNECTED  //  DISARMED")
        self.hud_status_lbl.set_name("hud_readout_dim")
        self.hud_status_lbl.set_xalign(0)
        hud_box.pack_start(self.hud_status_lbl, False, False, 0)

        self.hud_roll_lbl = Gtk.Label(label="ROLL   --")
        self.hud_roll_lbl.set_name("hud_readout")
        self.hud_roll_lbl.set_xalign(0)
        hud_box.pack_start(self.hud_roll_lbl, False, False, 0)

        self.hud_pitch_lbl = Gtk.Label(label="PITCH  --")
        self.hud_pitch_lbl.set_name("hud_readout")
        self.hud_pitch_lbl.set_xalign(0)
        hud_box.pack_start(self.hud_pitch_lbl, False, False, 0)

        self.hud_thrust_lbl = Gtk.Label(label=f"THR    {THRUST_MIN}")
        self.hud_thrust_lbl.set_name("hud_readout")
        self.hud_thrust_lbl.set_xalign(0)
        hud_box.pack_start(self.hud_thrust_lbl, False, False, 0)

        self.hud_revealer.add(hud_box)
        overlay.add_overlay(self.hud_revealer)

        # ---------------- Overlay: toggle buttons ----------------
        # Both added LAST (after every revealer) - keeps them topmost over
        # any panel in both z-order and click-handling, so they stay
        # clickable no matter which panels are open or closed.
        self.hud_toggle_btn = Gtk.Button(label="HUD")
        self.hud_toggle_btn.set_name("hud_toggle")
        self.hud_toggle_btn.set_halign(Gtk.Align.START)
        self.hud_toggle_btn.set_valign(Gtk.Align.START)
        self.hud_toggle_btn.set_margin_start(8)
        self.hud_toggle_btn.set_margin_top(8)
        self.hud_toggle_btn.connect("clicked", lambda *_: self._toggle_hud())
        overlay.add_overlay(self.hud_toggle_btn)
        ...
        self.show_all()
        # Keep both panels hidden after show_all() forces every widget visible
        self.control_revealer.set_reveal_child(False)
        self.hud_revealer.set_reveal_child(False)
    ...
    def _toggle_hud(self):
        showing = self.hud_revealer.get_reveal_child()
        self.hud_revealer.set_reveal_child(not showing)
    ...
    def _set_attitude_display(self, roll, pitch):
        def _apply():
            ...
            self.hud_roll_lbl.set_text(f"ROLL   {roll:.2f}°")
            self.hud_pitch_lbl.set_text(f"PITCH  {pitch:.2f}°")
            return False
    ...
    def _update_cmd_display(self):
        ...
        self.hud_thrust_lbl.set_text(f"THR    {self.setpoint['thrust']}")

    def _update_hud_status(self):
        conn = "CONNECTED" if self.connected else "DISCONNECTED"
        arm = "ARMED" if self.armed else "DISARMED"
        self.hud_status_lbl.set_text(f"{conn}  //  {arm}")
        self.hud_status_lbl.set_name("hud_readout" if (self.connected and self.armed) else "hud_readout_dim")
    ...
    def _build_video_pipeline(self):
        pipe_str = (
            f'souphttpsrc location="{CAM_URL}" is-live=true '
            f'! multipartdemux '
            f'! image/jpeg '
            f'! jpegdec '
            f'! videoconvert '

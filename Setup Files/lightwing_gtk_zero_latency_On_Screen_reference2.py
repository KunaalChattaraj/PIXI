...
class VideoTiltOverlayPainter:
    """OPT-IN pipeline-embedded version of the same tilt reference, drawn
    directly onto video frames via GStreamer's cairooverlay - only used
    when the "HUD on Video" toggle is on.

    This is the version that caused the freeze/disconnect: a per-frame
    Python draw callback on GStreamer's own thread has to acquire the GIL
    every frame, and once cflib's thread + the setpoint thread + telemetry
    GUI updates are all competing for it too, frames stall and the
    ESP32-CAM's HTTP server drops the slow reader.

    Kept available because it looks nicer for demos/recordings where lag
    doesn't matter - but the pipeline is only built with this element
    when the user explicitly opts in, and torn down/rebuilt without it
    the moment they opt back out. Default is OFF."""
...
class DroneVideoApp(Gtk.Window):
    ...
    def _build_ui(self):
        ...
        # Opt-in: bake the same reference onto the video itself. OFF by
        # default - this costs a per-frame Python draw callback on
        # GStreamer's thread, which competes for the GIL with the drone
        # link threads and can stall/disconnect the camera feed under load.
        # Rebuilds the pipeline when toggled, so the cost is only paid
        # while this is actually on.
        self.video_hud_check = Gtk.CheckButton(label="HUD on video (may cause lag)")
        self.video_hud_check.set_active(False)
